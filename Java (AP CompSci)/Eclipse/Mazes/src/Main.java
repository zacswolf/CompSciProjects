...
public class Main extends Canvas implements Runnable{
	...
	public ArrayList<Integer> numArray = new ArrayList<Integer>();
	...
	public void render(){
		...
		while (((WIDTH * SCALE) * (HEIGHT * SCALE)) /2 > rectedArea){
			...
			numArray.add(X);
			numArray.add(Y);
			numArray.add(X + Height);
			numArray.add(Y + Width);
			int thesize = numArray.size() - 4;
			int counter = 0;
			while(thesize / 4 > counter || thesize == 0){
				if (thesize == 0){
					thesize = -1;
				}
